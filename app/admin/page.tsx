...
export default function AdminLoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    // Simulate API call - check admin credentials
    setTimeout(() => {
      if (email === ADMIN_CREDENTIALS.email && password === ADMIN_CREDENTIALS.password) {
        ...
        setError("Invalid email or password. Please use the sample credentials.")
        setIsLoading(false)
      }
    }, 1000)
  }
        ...
        <Card className="backdrop-blur-xl bg-white/95 shadow-2xl border-0 overflow-hidden">
          ...
          <div className="relative bg-gradient-to-br from-purple-600 via-blue-600 to-indigo-700 p-8 text-white">
            ...
            >
              ...
              >
                Sign in to access the Nthoppa Admin Dashboard
              </motion.p>
            </motion.div>
          </div>
            ...
            <form onSubmit={handleLogin} className="space-y-6">
              ...
              >
                ...
                <Input
                  id="email"
                  type="email"
                  placeholder="admin@nthoppa.com"
                  value={email}
                  onChange={(e) => setEmail(e.target.value)}
                  required
                  className="h-12 border-2 border-gray-200 focus:border-purple-600 focus:ring-2 focus:ring-purple-600/20 transition-all"
                />
              </motion.div>
              ...
              >
                <Button
                  type="submit"
                  className="w-full h-12 bg-gradient-to-r from-purple-600 to-indigo-700 hover:from-purple-700 hover:to-indigo-800 text-white font-bold shadow-lg hover:shadow-xl transition-all duration-300 transform hover:scale-[1.02]"
                  disabled={isLoading}
                >
                  {isLoading ? (
                    <>
              ...
              >
                <p className="text-sm font-semibold text-blue-900 mb-2">Sample Admin Credentials:</p>
                <div className="space-y-1">
                  <p className="text-xs text-blue-700">
                    <strong>Email:</strong> admin@nthoppa.com
                  </p>
                  <p className="text-xs text-blue-700">
